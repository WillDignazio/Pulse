...
public class FiberStreamDecoder extends Reader {
       ...
       throws UnsupportedEncodingException, SuspendExecution
   {
       ...
       try {
           if (Charset.isSupported(csn))
               return new FiberStreamDecoder(in, lock, Charset.forName(csn));
   ...
   public static FiberStreamDecoder forDecoder(ReadableByteChannel ch,
                                          CharsetDecoder dec,
                                          int minBufferCap) throws SuspendExecution
   {
       return new FiberStreamDecoder(ch, dec, minBufferCap);
   }
   ...
   // -- Public methods corresponding to those in InputStreamReader --
    ...
    private int read0() throws IOException, SuspendExecution {
    ...
    private static FileChannel getChannel(FileInputStream in) throws SuspendExecution {
    ...
    FiberStreamDecoder(ReadableByteChannel ch, CharsetDecoder dec, int mbc) {
        this.in = null;
        this.ch = ch;
        this.decoder = dec;
        this.cs = dec.charset();
    ...
    private int readBytes() throws IOException, SuspendExecution {
        bb.compact();
        try {
        if (ch != null) {
    ...
    int implRead(char[] cbuf, int off, int end) throws IOException, SuspendExecution {
    ...
    private boolean inReady() throws SuspendExecution{
        try {
    ...
    boolean implReady() throws SuspendExecution {
